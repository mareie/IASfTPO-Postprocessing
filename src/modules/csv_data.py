...
class CsvData:
    ...
    def save_to_csv(self, output_path):
        ...
        with open(output_path, "w", encoding="utf-8-sig", newline="") as f:
            f.write(self.sep.join(map(str, sim_header)) + "\n")
            f.write(self.sep.join(map(str, sim_values)) + "\n")
            f.write(self.sep.join(map(str, cols)) + "\n")
            f.write(self.sep.join(map(str, info_row)) + "\n")
            f.write(self.sep.join(map(str, desc_row)) + "\n")
            self.df.to_csv(
                f, index=False, header=False, sep=self.sep, lineterminator="\n"
            )
